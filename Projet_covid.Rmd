...
De plus, la médiane est de 12 174 alors que la moyenne est de 32 315 nouveaux cas par jour. Ceci nous prouve que certains jours présentent des valeurs extrêmes qui impacte la moyenne à la hausse.
...
Nous allons donc différencier la série avec la fonction diff. Elle va nous permettre de calculer la différence entre chaque observation et la précédente. Notre objectif est rendre stationnaire la série grâce à cette méthode.
...
ndiffs(serie_diff) # la série a été différencié comme il faut
```
...
En conclusion, notre série n'est pas **stationnaire**. Cette conclusion nous permet de confirmer que la mise en place d'un modèle linéaire n'est pas une solution pour modéliser notre série. Notre série n'étant pas stationnaire, nos différents modèles seront moins fiables et nos prévisions moins précises. 

## Lissage de la série

Puisque notre série n'est pas stationnaire, nous allons essayer d'appliquer un filtre moyenne mobile pour éliminer les fluctuations à court terme.  Nous allons éliminer le bruit à court terme pour mettre en évidence les tendances à plus long terme. 

```{r}
lissage <- c(.5,rep(1,6),.5)/7
serie_lissage <- serie_diff |> 
  stats::filter(filter=lissage,sides=2)
ggtsdisplay(serie_lissage,plot.type = "scatter")
```

```{r}
kpss.test(na.omit(serie_lissage))
adf.test(na.omit(serie_lissage))
```
Ce filtre ne rend toujours pas notre série stationnaire, en effet, les p-value nous permettent d'admettre l'hypothèse de non-stationnarité dans les deux cas. Nous allons donc continuer l'étude en considérant que notre série chronologique n'est pas stationnaire. 
...
On l'a déjà fait c'est avec auto.arima()

Si vous voulez qu'on le refasse quand même : 
...
Finalement, notre analyse de l'évolution du nombre de Covid-19 entre 2021 et 2023 nous permet d'affirmer que les prévisions sont compliquées à mettre en place. En effet, ayant une série non stationnaire avec une moyenne et une variance variant dans le temps, la modélisation est d'autant moins fiable. De plus, lors de cette période, nous avons eu des changements structurels avec la mise en place de confinements et de politiques sanitaires spécifiques à la pandémie, pouvant impacter le nombre de nouveaux cas de covid 19. Ce sont des paramètres que nous ne pouvons prévoir mais qui sont très importants dans les modélisations sur le nombre de cas du covid 19. L'analyse de la série aurait été plus fiable et simple si la série avait été stationnaire.
Par ailleurs, sur l'analyse de notre sous-série, nous avons pu conclure d'une saisonnalité hebdomadaire, avec un nombre de nouveaux cas en hausse tous les 7 jours.
