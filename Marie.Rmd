...
Notre étude commence donc le 1er Mars 2020 jusqu'au 22 décembre 2021.

Pour rappel, notre série présente une tendance à la hausse et une saisonnalité, mais celle-ci n'est pas régulière. En effet, les différentes hausses de nouveaux cas de covid dépendent des confinements et des mesures sanitaires mises en place.
...
Nous allons essayer de transformer la série pour stabiliser la croissance exponentielle du nombre de nouveaux cas.

```{r}
ts_serie1_log <- log(ts_serie1)
ts_serie1_log
length(ts_serie1_log)
length(ts_serie1)
