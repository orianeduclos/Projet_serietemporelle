...
Nous devons donc avoir 2 régresseurs pour la tendance et 6 régresseurs pour la saisonnalité. \
...
```{r}
# # Initialiser les valeurs d'AIC
# min_aic <- Inf # permet de suivre la valeur minimum de l'AIC rencontrée lors de l'itération des différentes valeurs de p
# best_p <- 0 # ariable sera mise à jou
# 
# # Tester différentes valeurs de p
# for (p in 1:10) {
#   model <- arima(serie_diff, order=c(p, 0, 0))
#   aic <- AIC(model)
#   
#   if (aic < min_aic) {
#     min_aic <- aic
...
# 
# 
# cat("Valeur de p identifiée :", best_p, "\n")
# 
# model_ar <- ar(serie_diff, order=best_p)
...
Construisons un modèle ARMA à partir de notre série différenciée. 

Si nous reutilisons les valeurs de p et q identifier plus haut.

```{r}
model_arma <- arima(serie_diff, order=c(0, 0, 3))
```
...
```{r,results='hold'}
c(AIC(model_arima),AIC(modele_arima1),AIC(ts_serie1_lm), AIC(model_arma))
c(BIC(model_arima),BIC(modele_arima1),BIC(ts_serie1_lm), BIC(model_arma))
```
...
Ce qui suggère que l'écart entre la distribution empirique et la distribution théorique est statistiquement significatif.
...
```{r}
les <- ets(ts_serie_semaine,model="ANN") # méthode de lissage exponentiel triple (ETS) avec un modèle de réseau de neurones artificiels (ANN).
pred <- predict(les) #génère des prédictions à l'aide du modèle ETS entraîné précédemment
plot(pred) #trace un graphique des prédictions 
```

# A INTERPRETER !!

Le code, ci dessus, ajuste un modèle ETS avec un modèle de réseau de neurones artificiels à une série temporelle ts_serie_semaine, puis génère des prédictions à l'aide de ce modèle et les affiche sur un graphique. Ici, sur le graphique on l'observe en bleu. 
...
```{r}
ts_serie_semaine2 <- ts(ts_serie2, frequency = 7)
autoplot(ts_serie_semaine2) # Visualisation des données
```


```{r}
pacf(ts_serie_semaine2)
```
Il faut différencier la série 3 fois.

```{r}
serie_diff2 <- ts_serie_semaine2 |> 
  diff(differences = 3) 
serie_diff2 |> 
  ggtsdisplay(plot.type="scatter")
```
...
```{r}
plot(forecast_cases)
lines(serie_lissage,col='red',lwd=3) # ON VOIT BIEN ICI QUE LA LIGNE ROUGE EST INVISIBLE APRES

#lines(serie_diff2,col="red", lwd = 2, lty = "dashed")
#lines(serie_diff2,col="red", lwd = 2, lty = "dashed", xlim =c(100,120))


# Définissez les coordonnées de départ de la ligne
x_start <- 500
y_start <- 50000

# Tracez la ligne
lines(x = c(x_start, 10), y = c(y_start, 10), col = "red")
